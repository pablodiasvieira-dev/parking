    ...
    return (
        <>
            <div className='item-central mx-7 h-full flex justify-center items-center'>
                {props.statusVacancy === "lock" && (<LockKeyholeIcon className={`text-red-500 dark:text-primary-foreground`} />)}
                {props.statusVacancy === "use" && (<IconCar isRight = {props.isRight}  isSelect = {props.isSelect} />)}
                {props.statusVacancy === "reserved" && (<Clock10Icon className={`${props.isSelect ? 'text-primary-foreground/90': 'text-primary'}  dark:text-primary-foreground`}/>)}
